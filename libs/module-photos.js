...
// @version      7.2
// @description  Профессиональное КП с настройками, уменьшенной шапкой и правкой марки/модели
...
(function() {
    ...
    waitForHub(() => {
        ...
        let companySettings = {
            ...
            // Ручные переопределения марки и модели
            customBrand: '',
            customModel: ''
        };
        ...
        // Функция диалога настроек (будет встроена в HTML)
        function getSettingsDialogScript() {
            return `
                <script>
                (function() {
                    // Загружаем текущие настройки из localStorage
                    let settings = JSON.parse(localStorage.getItem('encar_company_settings') || '{}');
                    ...
                    function showModal() {
                            ...
                            <div id="settings-overlay" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:20000;display:flex;align-items:center;justify-content:center;font-family:'Segoe UI',sans-serif;">
                                <div style="background:#1e293b;border-radius:20px;padding:25px;width:500px;max-width:90%;color:#f1f5f9;box-shadow:0 20px 40px rgba(0,0,0,0.5);max-height:90vh;overflow-y:auto;">
                                    ...
                                    <div style="margin-bottom:12px;">
                                        ...
                                        <input type="text" id="set-companyName" value="${settings.companyName.replace(/"/g, '&quot;')}" style="width:100%;padding:8px;border-radius:8px;border:none;background:#0f172a;color:white;">
                                    </div>
                                    <div style="margin-bottom:12px;">
                                        <label style="display:block;font-size:12px;margin-bottom:4px;color:#94a3b8;">ИНН</label>
                                        <input type="text" id="set-inn" value="${settings.inn}" style="width:100%;padding:8px;border-radius:8px;border:none;background:#0f172a;color:white;">
                                    </div>
                                    <div style="margin-bottom:12px;">
                                        <label style="display:block;font-size:12px;margin-bottom:4px;color:#94a3b8;">ОГРН</label>
                                        <input type="text" id="set-ogrn" value="${settings.ogrn}" style="width:100%;padding:8px;border-radius:8px;border:none;background:#0f172a;color:white;">
                                    </div>
                                    <div style="margin-bottom:12px;">
                                        <label style="display:block;font-size:12px;margin-bottom:4px;color:#94a3b8;">Адрес</label>
                                        <input type="text" id="set-address" value="${settings.address.replace(/"/g, '&quot;')}" style="width:100%;padding:8px;border-radius:8px;border:none;background:#0f172a;color:white;">
                                    </div>
                                    <div style="margin-bottom:12px;">
                                        <label style="display:block;font-size:12px;margin-bottom:4px;color:#94a3b8;">Телефон компании</label>
                                        <input type="text" id="set-phone" value="${settings.phone}" style="width:100%;padding:8px;border-radius:8px;border:none;background:#0f172a;color:white;">
                                    </div>
                                    <div style="margin-bottom:12px;">
                                        <label style="display:block;font-size:12px;margin-bottom:4px;color:#94a3b8;">Имя менеджера</label>
                                        <input type="text" id="set-managerName" value="${settings.managerName.replace(/"/g, '&quot;')}" style="width:100%;padding:8px;border-radius:8px;border:none;background:#0f172a;color:white;">
                                    </div>
                                    <div style="margin-bottom:12px;">
                                        <label style="display:block;font-size:12px;margin-bottom:4px;color:#94a3b8;">Должность менеджера</label>
                                        <input type="text" id="set-managerPosition" value="${settings.managerPosition.replace(/"/g, '&quot;')}" style="width:100%;padding:8px;border-radius:8px;border:none;background:#0f172a;color:white;">
                                    </div>
                                    <div style="margin-bottom:12px;">
                                        <label style="display:block;font-size:12px;margin-bottom:4px;color:#94a3b8;">Телефон менеджера</label>
                                        <input type="text" id="set-managerPhone" value="${settings.managerPhone}" style="width:100%;padding:8px;border-radius:8px;border:none;background:#0f172a;color:white;">
                                    </div>
                                    <div style="margin-bottom:12px;">
                                        <label style="display:block;font-size:12px;margin-bottom:4px;color:#94a3b8;">Срок действия (дней)</label>
                                        <input type="number" id="set-offerValidDays" value="${settings.offerValidDays}" style="width:100%;padding:8px;border-radius:8px;border:none;background:#0f172a;color:white;">
                                    </div>
                                    <div style="margin-bottom:12px;">
                                        <label style="display:block;font-size:12px;margin-bottom:4px;color:#94a3b8;">URL логотипа</label>
                                        <input type="text" id="set-logo" value="${settings.logo}" style="width:100%;padding:8px;border-radius:8px;border:none;background:#0f172a;color:white;">
                                    </div>
                                    <div style="margin-bottom:12px;">
                                        <label style="display:block;font-size:12px;margin-bottom:4px;color:#94a3b8;">Марка (оставьте пустым для авто-определения)</label>
                                        <input type="text" id="set-customBrand" value="${settings.customBrand || ''}" style="width:100%;padding:8px;border-radius:8px;border:none;background:#0f172a;color:white;" placeholder="Например: BMW">
                                    </div>
                                    <div style="margin-bottom:12px;">
                                        <label style="display:block;font-size:12px;margin-bottom:4px;color:#94a3b8;">Модель (оставьте пустым для авто-определения)</label>
                                        <input type="text" id="set-customModel" value="${settings.customModel || ''}" style="width:100%;padding:8px;border-radius:8px;border:none;background:#0f172a;color:white;" placeholder="Например: X6">
                                    </div>
                        ...
                        document.getElementById('settings-save').onclick = saveAndReload;
                    }
                    
                    window.showCompanySettings = showModal;
                })();
                </script>
            `;
        }
        ...
        async function printReport() {
            ...
            // Определяем итоговые марку и модель (с учётом ручного переопределения)
            ...
            // Подготовим скрипт диалога настроек
            const settingsScript = getSettingsDialogScript();
...
<head>
    ...
    <style>
        * {
        ...
        /* Остальные стили без изменений (крупные шрифты) */ 
        .section {
            padding: 20px 30px;
...
<body>
<div class="toolbar">
    <button class="btn-settings" onclick="showCompanySettings()">⚙️ Настройки</button>
    ...
    <div class="page">
        ...
        <div class="section">
            ...
            <div style="margin-bottom:16px;">
                <div style="font-weight:800; margin-bottom:8px; font-size:15px; color:#1e3a5f;">🇰🇷 Расходы Корея</div>
            ...
            <div style="margin-bottom:16px;">
                <div style="font-weight:800; margin-bottom:8px; font-size:15px; color:#1e3a5f;">🇰🇬 Расходы Бишкек</div>
                <div class="expense-detail">
            ...
            <div style="margin-bottom:16px;">
                <div style="font-weight:800; margin-bottom:8px; font-size:15px; color:#1e3a5f;">🇷🇺 Расходы РФ</div>
                <div class="expense-detail">
            ...
            <div style="margin-bottom:16px;">
                <div style="font-weight:800; margin-bottom:8px; font-size:15px; color:#1e3a5f;">🏛️ Таможенное оформление</div>
                <div class="expense-detail">
...
${settingsScript}
        ...
        unsafeWindow.EncarPhotos = { 
            ...
            showSettings: () => {} // заглушка, не используется
        };
        
        console.log('[Photos] Модуль загружен v7.2 (уменьшенная шапка, редактирование марки/модели)');
    });
})();
