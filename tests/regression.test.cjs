...
test('TPO converts 48 percent of EUR once, with cents and rate updates',()=>{const e=calc();e.hub.set('manualTpo',null);e.hub.set('selectedEuroPrice',30000);e.hub.set('eurUsdRate',1.16);assert.equal(e.data.calculatedTpo,16704);e.hub.set('eurUsdRate',1.1);assert.equal(e.data.calculatedTpo,15840);e.hub.set('selectedEuroPrice',30000.33);assert.equal(e.data.calculatedTpo,15840.17);});
test('manual EUR/USD survives automatic updates and can be reset',()=>{const e=calc();e.hub.set('manualTpo',null);e.hub.set('selectedEuroPrice',30000);e.hub.set('manualEurUsdRate',1.2);e.hub.set('eurUsdRate',1.1);assert.equal(e.data.calculatedTpo,17280);e.hub.set('manualEurUsdRate',null);assert.equal(e.data.calculatedTpo,15840);});
